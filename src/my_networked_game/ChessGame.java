...
public class ChessGame implements Serializable{
		...
		for (int i=0; i < pieces.length; i++)
		{
			if (pieces[i].areYouHere(xSelectLoc, ySelectLoc))
			{
				if(clientIndex == nextTurn){
					if((nextTurn%2 == 0 && pieces[i].color.equals(ColorType.white))|| 
							(nextTurn%2 == 1 && pieces[i].color.equals(ColorType.black))){
	...
	void mouseReleased(DPoint dpoint)
	{
		...
		if (selectedIndex >= startingTurn)
		{
			int xSelectLoc = (int)dpoint.x;
			int ySelectLoc = (int)dpoint.y;
			if (xSelectLoc >= 0 && ySelectLoc >= 0 && pieces[selectedIndex].isLegalMove(pressed_point, dpoint, pieceArray)) 
			{
				if(pieceArray[xSelectLoc][ySelectLoc] != null){
					if(pieceArray[xSelectLoc][ySelectLoc].pieceType.equals(PieceType.King)){
